...
class GroundTruthEvaluator:
    ...
    def evaluate(
        ...
        for gt_turn in ground_truth_turns:
            ...
            gt_spk = gt_turn.get("speaker", "SPEAKER_01")
        ...
        missed_word_rate = (
            sum(len(self._extract_missing_words(gt.get("text", ""), (system_outputs[idx].get("text", "") if idx < len(system_outputs) else ""))) for idx, gt in enumerate(ground_truth_turns))
            / max(1, total_ref_words)
        )
